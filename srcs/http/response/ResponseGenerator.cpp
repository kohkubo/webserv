...
Response ResponseGenerator::generate_response() {
  ...
  if (!_is_error_status_code(_response_info_.status_code_)) {
    ...
    if (pair.first) {
      _response_info_.status_code_ = pair.second;
    }
  }
  if (!_is_error_status_code(_response_info_.status_code_) &&
      _response_info_.status_code_ != HttpStatusCode::MOVED_PERMANENTLY_301) {
  ...
  if (!body.has_content_) {
    body.content_ = _create_default_body_content(_response_info_.status_code_);
  }
  // LOG("status_code: " << _response_info_.status_code_);
