...
HttpStatusCode
ResponseGenerator::__handle_post_method(const Location    &location,
                                        const std::string &file_path) {
...
HttpStatusCode
ResponseGenerator::__handle_get_method(const Location    &location,
                                       const std::string &file_path) {
  // TODO: ここらへんの処理、未定なので雑に書いています。
  if (std::find(location.limit_except_.begin(), location.limit_except_.end(),
...
HttpStatusCode
ResponseGenerator::__handle_delete_method(const Location    &location,
                                          const RequestInfo &request_info,
                                          const std::string &file_path) {
  // TODO: ここらへんの処理、未定なので雑に書いています。
  if (std::find(location.limit_except_.begin(), location.limit_except_.end(),
