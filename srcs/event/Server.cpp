...
Server::Server(const char *config_file_path) {
  _read_config(config_file_path);
  _socket_map_ = _generate();
}

void Server::_read_config(const char *config_file_path) {
...
std::map<int, SocketBase *> Server::_generate() {
...
std::vector<struct pollfd> Server::_create_pollfds() {
...
void Server::_close_timedout_socket() {
  std::map<int, SocketBase *>::const_iterator it = _socket_map_.begin();
...
void Server::run_loop() {
  ...
  for (;;) {
    _close_timedout_socket();
    std::vector<struct pollfd> pollfds = _create_pollfds();
