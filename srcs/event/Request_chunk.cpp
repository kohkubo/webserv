...
Request::RequestState Request::_chunk_loop(std::string &request_buffer) {
  std::string line;
  while (get_next_chunk_line(_chunk_.next_chunk_type_, request_buffer, line,
                             _chunk_.next_chunk_size_)) {
    if (_chunk_.next_chunk_type_ == CHUNK_SIZE) {
      Result<std::size_t> result = hexstr_to_size(line);
      if (result.is_err_) {
        throw RequestInfo::BadRequestException();
      }
      _chunk_.next_chunk_size_ = result.object_;
      _chunk_.next_chunk_type_ = CHUNK_DATA;
    } else {
      bool is_last_chunk = _chunk_.next_chunk_size_ == 0 && line == "";
      if (is_last_chunk) {
        return SUCCESS;
      }
      _request_info_.content_.append(line);
